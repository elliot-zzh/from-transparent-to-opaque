...
def get_best_param_score(score_file, param_name):
    if not os.path.exists(score_file):
        print(f"Score file {score_file} does not exist. Using default value for {param_name}.")
        return None, None
    best_value = None
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python script_name.py <original_config.toml> <gpu_num> <searching_step>")
        sys.exit(1)
    ...
    # First phase: Tune lr
    lr_files = write_configs_to_files(generate_single_param_configs(original_config, 
    ...
    with open('train.sh', 'w') as f:
        ...
        f.write(f'best_lr=$(awk \'NR==1{{print $1}}\' {hyperparameters_to_tune[0]["score_filename"]} 2>/dev/null)\n')
        ...
        f.write(f'python -c "import toml; config = toml.load(\'{config_file}\'); config[\'training\'][\'lr\'] = float(os.environ.get(\'best_lr\', {hyperparameters_to_tune[0]["param_values"][0]})); toml.dump(config, \'final_best_config.toml\')"\n')

        # Tune other hyperparameters
        other_files = []
        for hp in hyperparameters_to_tune[1:]:
            configs = generate_single_param_configs(original_config, hp['param_name'], hp['param_values'])
