...
data = pl.scan_parquet('hf://datasets/open-thoughts/OpenThoughts-114k/data/train-*.parquet')  # lazy load


def combine_conversations(x):
    return (x[0]['value'] + '\n' + x[1]['value']).replace('<|begin_of_thought|>', '<think>').replace(
        '<|end_of_thought|>', '</think>').replace('<|begin_of_solution|>', '').replace('<|end_of_solution|>', '')
...
data = data.filter(
    pl.col('conversations').list.get(1).struct.field('value').str.len_chars() < max_string_length).collect()
data = data.with_columns(
    pl.col('conversations').map_elements(combine_conversations, return_dtype=pl.String).alias('text')
)
data = data.with_columns(
    pl.col('conversations').map_elements(gather_questions, return_dtype=pl.String).alias('questions')
)
data = data.with_columns(
    pl.col('conversations').map_elements(ans_pos, return_dtype=pl.Int16).alias('ans_pos')
)
