...
from vae.parameters import batch_size
...
class Data(Dataset):
    def __init__(self, data_raw):
        ...
        if len(data_raw) <= 10000:
            self.data = tokenizer(data_raw.tolist(), return_tensors='pt', truncation=True, padding='max_length',
                                  max_length=1536)
        else:
            data = data_raw[:len(data_raw) - len(data_raw) % 10000].reshape(-1, 10000).tolist() + [
                data_raw[len(data_raw) - len(data_raw) % 10000:].tolist()]
