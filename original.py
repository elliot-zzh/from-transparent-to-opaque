...
class Gate(nn.Module):
    ...
    def forward(self, hidden, embed):
        ...
        return embed * (1 - self.gate) + self.inject_scale * self.gate * hidden * self.time_mixing_gate(hidden, embed)
...
def sampler(input_ids, attn_mask, temperature=0.7, topk=16, max_length=2048, num=16, min_p=0.02, gc_interval=64,
            hidden_dropout_rate=0.02, depth=0):
    model.eval()
    vae.eval()
    gater.eval()
    ...
    with torch.amp.autocast(device_type=str(device), dtype=torch.float16):
        outputs = model(input_ids=input_ids, attention_mask=attn_mask, output_hidden_states=True, return_dict=True,
                        cache_position=cache_pos, past_key_values=kv_cache)
    ...
    text_end_indices = torch.ones(problem_batch_size, dtype=torch.long).to(device) * (max_length + input_ids.shape[1])
    ...
    for i in tqdm(range(max_length), desc='sampling progress'):
        with torch.cuda.stream(hidden_stream):
            with torch.amp.autocast(device_type=str(device), dtype=torch.float16):
                last_hidden = outputs.hidden_states[hidden_layer_num]
                hidden_cache = torch.cat([hidden_cache, F.dropout(vae(last_hidden[:, -1:, :], compressing=True),
                                                                  p=hidden_dropout_rate, training=True)], dim=1)
                ...
                if depth > 0:
                    ...
                    causal_mask = model.model.model._update_causal_mask(
                        attention_mask=attn_mask[:, input_ids.shape[1] + i - 1].unsqueeze(1), input_tensor=last_hidden,
                        cache_position=deep_cache_pos, past_key_values=deep_kv_cache[0], output_attentions=False
                    )
        ...
        selected_choice = torch.multinomial(probs.view(problem_batch_size, -1), num_samples=1)
        ...
        attn_mask = torch.cat([attn_mask, text_end_indices.unsqueeze(1)], dim=1)  # update attention mask
        ...
        with torch.amp.autocast(device_type=str(device), dtype=torch.float16):
            ...
            embeds = model.lm_head.weight[selected_index.view(problem_batch_size, 1)].to(device)
            torch.cuda.current_stream().wait_stream(hidden_stream)
            embeds = gater(uncompressed_hidden, embeds)
            outputs = model(inputs_embeds=embeds, attention_mask=attn_mask, output_hidden_states=True,
                            cache_position=cache_pos, return_dict=True, use_cache=True, past_key_values=kv_cache)
...
def verifier(model_anss, corr_anss, corr_score=2, wrong_score=-1):
    ...
    for idx, i in enumerate(model_anss):
        ...
        if model_ans:
            ...
            res.append(corr_score if verify(model_ans, parse(corr_anss[idx % len(corr_anss)])) else wrong_score)
        else:
...
optimizers = [AdamW(model.parameters(), lr=1e-5), AdamW(vae.parameters(), lr=5e-5), Adam(gater.parameters(), lr=3e-3)]
...
def linear_interpl(x: torch.Tensor, a: float, b: float, low: float,
                   high: float) -> torch.Tensor:  # only interpl between [a, b], linearly increase from low to high
    mask_low = x <= a
    mask_high = x >= b
    ...
    for problem, ans in data_train:
        ...
        with torch.no_grad():
            ...
            input_ids, problem_attn_mask = tokenize(
                sum([[prompt + i + prompt_suffix] * sample_num for i in problem], []), direct=True)
            for i in range(0, sample_problem_batch, sample_problem_sub_batch):
                if init_res:
                    res_, hidden_cache_, text_end_indices_, mask_ = sampler(
                        input_ids[i * sample_num:(i + sample_problem_sub_batch) * sample_num],
                        problem_attn_mask[i * sample_num:(i + sample_problem_sub_batch) * sample_num], num=sample_num,
                        topk=sample_topk, max_length=max_sample_length, temperature=sample_temperature,
                        depth=looping_depth)
                    ...
                    res, hidden_cache, text_end_indices, mask = sampler(
                        ...
                        problem_attn_mask[:sample_problem_sub_batch * sample_num], num=sample_num, topk=sample_topk,
                        max_length=max_sample_length, depth=looping_depth)
                    init_res = True
            ...
            correctness_rewards = torch.Tensor(
                verifier(tokenizer.batch_decode(res, skip_special_tokens=True), ans, corr_score=corr_reward)).to(device)
            len_rewards = text_end_indices.float() + 1

            # normalization
            filt = None
            if (l := (corr_filt := correctness_rewards == corr_reward).sum()) < res.shape[
                0] / 3 and l != 0:  # clip too many wrong answers, currently 1:1
            ...
            len_interval_mask = torch.logical_not(torch.logical_or(max_len_mask, cache_len_mask))
            if len_interval_mask.any():
                len_rewards[len_interval_mask] = (l_cache_length - len_rewards[len_interval_mask]) / (
                            max_sample_length - l_cache_length)
            rewards = correctness_rewards + len_rewards
            rewards = norm(rewards)

        if acc_check_only: continue
                ...
                try:
                    ...
                    with torch.amp.autocast(device_type=str(device), dtype=torch.float16):
                        last_hidden = vae.uncompress(
                            F.dropout(hidden_cache_slice, p=hidden_dropout_rate, training=True))
                        if looping_depth > 0:  # deep looping
                            hidden_pos = torch.arange(0, last_hidden.shape[1], dtype=torch.long, device=device)
                            causal_mask = model.model.model._update_causal_mask(
                                attention_mask=mask[i:end, input_ids.shape[1] - 1:-1], input_tensor=last_hidden,
                                cache_position=hidden_pos, output_attentions=False, past_key_values=None
                        ...
                        embeds = torch.cat(
                            [embeds[:, :input_ids.shape[1]], gater(last_hidden, embeds[:, input_ids.shape[1]:])], dim=1)
                        outputs = model(inputs_embeds=embeds, attention_mask=mask[i:end], output_hidden_states=True,
                                        return_dict=True)
                        loss = lossf(outputs.logits[:, input_ids.shape[1] - 1:].transpose(1, 2),
                                     seqs[i:end, input_ids.shape[1]:].masked_fill(mask[i:end, input_ids.shape[1]:] == 0,
                                                                                  -100))
                        ...
                        if looping_depth > 0:  # deep looping
                            ...
                            causal_mask = model.model.model._update_causal_mask(
                                attention_mask=mask[i:end, input_ids.shape[1] - 1:-1], cache_position=hidden_pos,
                                input_tensor=new_processed_hidden, output_attentions=False, past_key_values=None
                            )  # the mask here needs to be re-considered
                        ...
                        new_processed_hidden = gater.forward_hidden(new_processed_hidden,
                                                                    embeds[:, input_ids.shape[1]:])
                        processed_hidden = gater.forward_hidden(last_hidden, embeds[:, input_ids.shape[1]:])
                        hidden_loss = hidden_regularizer(new_processed_hidden, processed_hidden).mean(dim=-1) * mask[
                                                                                                                i:end,
                                                                                                                input_ids.shape[
                                                                                                                    1]:-1]
                        # apply hidden regularization bonus
                        hidden_loss = hidden_loss * linear_interpl((text_end_indices + 1)[i:i + batch_size],
                                                                   hidden_reg_len_bonus_a, max_sample_length, 1,
                                                                   hidden_reg_len_bonus_high)
                        # apply gating value bonus
                        gate_bonus = torch.exp(gating_value_lambda * (0.5 - gater.gate) ** 2).mean()
                        loss = ((loss.sum(dim=-1) * rewards[
                                                    i:end]).sum() + hidden_loss.sum() * hidden_regularization_rate) / (
                                           text_end_indices[i:i + batch_size] + 1).sum()
                        loss += gate_bonus * gating_value_bonus * gating_value_decay ** (
                        ...
                        with torch.no_grad():
                            update_index = torch.nonzero(
                                torch.randn(hidden_cache.shape[1], device=device) < hidden_updating_rate)
