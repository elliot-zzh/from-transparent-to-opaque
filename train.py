...
def sampler(input_ids, attn_mask, temperature=0.7, topk=16, max_length=2048, num=16, min_p=0.02, gc_interval=64, hidden_dropout_rate=0.02, depth=0):
    ...
    with torch.amp.autocast(device_type=str(device), dtype=torch.float16):
        outputs = model(input_ids=input_ids, attention_mask=attn_mask, output_hidden_states=True, return_dict=True, cache_position=cache_pos, past_key_values=kv_cache)
...
def linear_interpl(x: torch.Tensor, a: float, b: float, low: float, high: float) -> torch.Tensor: # only interpl between [a, b], linearly increase from low to high
    mask_low = x <= a
    mask_high = x >= b
...
while step <= total_steps:
    for problem, ans in data_train:
        ...
        with torch.no_grad():
            ...
            correctness_rewards = torch.Tensor(verifier(tokenizer.batch_decode(res, skip_special_tokens=True), ans, corr_score=corr_reward)).to(device)
            ...
            if len_interval_mask.any():
                len_rewards[len_interval_mask] = (l_cache_length - len_rewards[len_interval_mask]) / (max_sample_length - l_cache_length)
